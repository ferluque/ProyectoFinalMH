//
// Created by Fernando on 18/06/2022.
//
...
Solution BMO_memetico(Problem p);

int main () {
    Random::seed(0);

//    Problem p("..\\datos\\GKD-b_1_n25_m2.txt");
//    Solution s = BMO(p);
//    cout << s << endl;

    string path = "..\\datos";
    ofstream salida;
    salida.open("..\\resultados\\BMO_memetico.csv");
    for (const auto &entry: std::filesystem::directory_iterator(path)) {
        string file = entry.path().u8string();
        salida << file << ";";

        cout << "Empieza " << file << endl;
        Problem p(file);
        auto tinicio = clock();
        Solution S = BMO_memetico(p);
        auto tfin = clock();
...
Solution BMO_memetico(Problem p) {
    ...
    int generaciones = 0, evaluaciones = 0;
    while (evaluaciones < 1e4) {
        if ((generaciones % 10 == 0) && (generaciones / 10 >= 1)) {
            int mejor10 = 0.1 * inicial.size();
            for (int i = 0; i < mejor10; i++) {
                pair<Solution, int> par = bl(p, inicial[i]);
                inicial[i] = par.first;
                evaluaciones += par.second;
            }
            SolutionCompare mycomp;
            sort(inicial.begin(), inicial.end(), mycomp);
        } else {
