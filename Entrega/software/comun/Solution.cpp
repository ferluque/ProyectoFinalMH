...
vector<Solution> GeneraPoblacion(int M, Problem problema) {
    ...
    for (int i = 0; i < Padres.size(); i++) {
        ...
        pos = 0;
        for (int j = 0; j < problema.get_m(); j++) {
...
Solution Solution::cruce_multiple(const vector<Solution>& padres, const Problem& p) const {
    vector<bool> h(p.get_n(), false);
    vector<int> seleccionados = Random::get<vector>(0,static_cast<int>(padres.size()),p.get_m());
    for (int i=0; i<seleccionados.size(); i++) {
        if (seleccionados[i]==0)
            h[seleccionados[i]] = selected[i];
        else
            h[seleccionados[i]] = padres[seleccionados[i]-1].selected[i];
    }
    return (Solution(repare(h,p),p));
}

Solution Solution::neighbor(int i, int j, const vector<vector<float>>& d) const {
    // Asumimos aquí ya que el valor de selected[i] y selected[j] son diferentes
    Solution vecino(*this);

    bool aux = vecino.selected[i];
    vecino.selected[i] = vecino.selected[j];
    vecino.selected[j] = aux;
    // Si ahora es el xi el que pasa a ser 1 (antes xj era 1)
    if (vecino.selected[i]) {
        // Eliminamos el xj de los deltas
        // Antes de eliminar el xj de los deltas, debemos restarlo a los otros deltas
        for (auto it = vecino.deltas.begin(); it != vecino.deltas.end(); ++it) {
            if ((*it).first != j) {
                (*it).second -= d[j][(*it).first];
            }
        }
        vecino.deltas.erase(j);

        // Ahora ya añadimos el xi
        vecino.deltas[i] = vecino.delta(i, d);
        // Y sumamos el delta que añade al resto
        for (auto it = vecino.deltas.begin(); it != vecino.deltas.end(); ++it) {
            if ((*it).first != i) {
                (*it).second += d[i][(*it).first];
            }
        }
    }
        // Si es el xj el que pasa a ser 1
        ...
        vecino.deltas.erase(i);

        // Ahora ya añadimos el xj
        vecino.deltas[j] = vecino.delta(j, d);
        // Y sumamos el delta que añade al resto
        for (auto it = vecino.deltas.begin(); it != vecino.deltas.end(); ++it) {
            if ((*it).first != j) {
                (*it).second += d[j][(*it).first];
